...
def downloadDetails(game_id):
    ...
    window_time = datetime.strptime(windowData['frames'][-1]['rfc460Timestamp'].split('.')[0].split('Z')[0] + 'Z', time_format)
    # Round to the nearest 10 seconds per API requirements
    window_time = window_time - timedelta(minutes=0, \
    ...
    while windowData['frames'][-1]['gameState'] != 'finished':
        ...
        windowData = requests.get(windowApi, params=params).json()
        detailData = requests.get(detailApi, params=params).json()
        ...
        window_time = window_time + timedelta(seconds=10)
...
def downloadMeta(query, params={}):
    ...
    params['hl'] = 'en-US'
    response = requests.get(api + query, headers=requestHeader, params=params)
